        ...
        <div v-for="model in chatModels" :key="model.id" class="model-card" :class="{ 'builtin-model': model.isBuiltin }">
          <div class="model-info">
            <div class="model-name">
              {{ model.name }}
              <t-tag v-if="model.isBuiltin" theme="primary" size="small">{{ $t('modelSettings.builtin.label') }}</t-tag>
            </div>
...
const loadModels = async () => {
  ...
  try {
    ...
    MessagePlugin.error(error.message || t('modelSettings.toasts.listFailed'))
  } finally {
    loading.value = false
  }
}
...
const editModel = (type: 'chat' | 'embedding' | 'rerank' | 'vllm', model: any) => {
  // 内置模型不能编辑
  if (model.isBuiltin) {
    MessagePlugin.warning(t('modelSettings.toasts.builtinNoEdit'))
    return
  }
  currentModelType.value = type
...
const deleteModel = async (type: 'chat' | 'embedding' | 'rerank' | 'vllm', modelId: string) => {
  ...
  if (model?.is_builtin) {
    MessagePlugin.warning(t('modelSettings.toasts.builtinNoDelete'))
    return
  }
