...
// 这样用户可以移除这些知识库
watch(
    [selectedAgentId, agentKnowledgeBases],
    ([newAgentId, newAgentKbs], [oldAgentId]) => {
...
const loadKnowledgeBases = async () => {
    try {
        ...
        if (response.data && Array.isArray(response.data)) {
            const validKbs = response.data.filter(
                (kb: any) =>
...
const loadMentionItems = async (q: string, resetIndex = true, append = false) => {
    ...
    if (!append) {
        ...
        const kbs = availableKbs.filter(
            kb => !q || kb.name.toLowerCase().includes(q.toLowerCase())
        );
        kbItems = kbs.map(kb => ({
            id: kb.id,
            name: kb.name,
...
const selectAgentMode = (mode: 'quick-answer' | 'smart-reasoning') => {
    ...
    if (builtinAgent) {
        const notReadyReasons = getBuiltinAgentNotReadyReasons(
            builtinAgent,
    ...
    if (shouldEnableAgent !== isAgentEnabled.value) {
        ...
        settingsStore.selectAgent(
            shouldEnableAgent ? BUILTIN_SMART_REASONING_ID : BUILTIN_QUICK_ANSWER_ID
        );
...
const handleGoToWebSearchSettings = () => {
    uiStore.openSettings('web_search');
                ...
                >
                    <template #content>
                        <div v-if="isWebSearchDisabledByAgent" class="tooltip-with-link">
                            ...
                            <a href="#" @click.prevent="handleGoToAgentSettings('web_search')">{{
                                $t('input.goToAgentSettings')
                            }}</a>
                        </div>
                        <span v-else-if="isWebSearchConfigured">{{
                            isWebSearchEnabled
                                ? $t('input.webSearch.toggleOff')
                                : $t('input.webSearch.toggleOn')
                        }}</span>
                        <div v-else class="tooltip-with-link">
                            <span>{{ $t('input.webSearch.notConfigured') }}</span>
                            <a href="#" @click.prevent="handleGoToWebSearchSettings">{{
                                $t('input.goToSettings')
                            }}</a>
                        </div>
                    </template>
                    ...
                    >
                        ...
                        >
                            ...
                            />
                            <path
                                d="M 9 2 A 3.5 7 0 0 0 9 16"
                                stroke="currentColor"
                                stroke-width="1.2"
                                fill="none"
                            />
                            <path
                                d="M 9 2 A 3.5 7 0 0 1 9 16"
                                stroke="currentColor"
                                stroke-width="1.2"
                                fill="none"
                            />
                            ...
                            >
                                <div class="model-option-main">
                                    ...
                                    <span
                                        ...
                                        class="model-badge-remote"
                                        >{{ $t('input.remote') }}</span
                                    >
                                    <span
                                        v-else-if="model.parameters?.parameter_size"
                                        class="model-badge-local"
                                    >
...
.rich-input-container {
    ...
    box-shadow:
        ...
        0 12px 12px -1px rgba(0, 0, 0, 0.08);
